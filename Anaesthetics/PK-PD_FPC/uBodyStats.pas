...
{*******************************************************************************
     ...
     Body-size formulas (BMI/BSA/LBW/IBW/adjusted-BW) and the Eleveld
     propofol/remifentanil population PK/PD model's own weight/age scaling
     functions. Pure scalar arithmetic - no matrix dependency at all, so this
     is a near-verbatim port of Anaesthetics/PK-PD/uBodystats.pas: the only
     change is TDate -> TDateTime (FPC's SysUtils has no TDate, unlike
     Delphi's System.pas) and the {$mode delphi} pragma this whole project
     builds under (see uModel3Comp.pas's header comment for why Delphi mode
     is required, same reason as newVM.pas's own operator overloads).
...
type
  ...
  Function AgeFromDOB(DOB : TDateTime):integer;
...
// Ideal Bodyweight - modified for use in the Eleveld model
