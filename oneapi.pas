...
{$LinkLib 'ipps.so'}
  {$ENDIF}
{$ENDIF}
{ Gated on HAVE_MKL/HAVE_IPP (from newVMConfig.inc, generated by
  ./newvmconfigure - see that file's own header comment) rather than
  unconditional, so a build on a machine without MKL/IPP installed doesn't
  fail at link time just from this unit's own {$Linklib}s - unchanged
  behaviour when both are present (as on this machine), which is still
  the only case newVMSingle.pas/newVMComplex.pas/newVMComplexSingle.pas
  support; see newVM.pas's PUREPASCAL fallback for the one unit that
  currently tolerates either missing. The external declarations below are
  still unconditional either way - only referenced (and thus only actually
  linked) by code that calls them, i.e. by newVM.pas's own non-PUREPASCAL
  branch. }
