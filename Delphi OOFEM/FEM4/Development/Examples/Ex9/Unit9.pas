...
begin
  ...
  Sto_ShellExecute('c:\gmsh\gmsh.exe', ['..\Data\sifona.geo', '-3'], ExitCode, 60000, True);

  Gmsh.OpenFile('..\Data\sifona.msh');
  Gmsh.ReadMesh;
  Gmsh.Close;
  ...
  begin
    ...
    if (Abs(Gmsh.CoordY[i] + 3) < 1E-3) then
    begin
      Gmsh.NodeBCIsFixed[i] := True;
      Gmsh.NodeBCValue[i] := -3;
    end;
  ...
  ShellExecute(Handle,'open', 'c:\gmsh\gmsh.exe', '..\Data\sifona.pos', nil, SW_SHOWNORMAL) ;
