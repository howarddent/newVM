...
begin
  ...
  Gmsh.OpenFile('..\Data\cxs.geo');
  //Gmsh.GenerateLine(1, 0.099, GMSH_BEAM);
  //Gmsh.GenerateRectangle(1, 1, 0.099, GMSH_TRI);
  //Gmsh.GenerateRectangle(1, 1, 0.099, GMSH_QUAD);
  //Gmsh.GenerateBox(1, 1, 1, 0.099, GMSH_TETRA);
  ...
  begin
    ...
    if (Abs(Gmsh.CoordY[i] - 1) < 1E-8) then
    begin
      IsFixed[i] := True;
      Values[i] := 1;
    end;
