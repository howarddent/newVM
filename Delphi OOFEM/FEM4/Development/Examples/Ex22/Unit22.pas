...
uses
  ...
  CXS.FEMLAP.Gmsh, CXS.FEMLAP.Assembly, CXS.FEMLAP.Analytical, CXS.FEMLAP.Face_T3V1, CXS.FEMLAP.Face_Q4V1;
...
begin
  ...
  begin
    ...
    Gmsh.OpenFile('..\Data\tbeam2D_' + Format('%.3d', [ti-1]) + '.pos');
  ...
  // its Parameters string - a literal 'tbeam2D_*.pos' argument reaches
  ...
  // so the zero-padded tbeam2D_NNN.pos sequence combines in the correct
  // time order.
  Files := TStringList.Create;
  try
    Files.Sorted := True;

    if FindFirst('..\Data\tbeam2D_*.pos', faAnyFile, SR) = 0 then
    begin
      repeat
        Files.Add(SR.Name);
      until FindNext(SR) <> 0;
    ...
    begin
      ShowMessage('No tbeam2D_*.pos result files found in ..\Data - run Calculate first.');
      Exit;
    end;
