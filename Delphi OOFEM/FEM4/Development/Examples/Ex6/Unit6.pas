...
uses
  ...
  Dialogs, ShellApi, StdCtrls, newVM, newVMsparse, CXS.FEMLAP.ShellExec,
...
begin
  ...
  Sto_ShellExecute('c:\gmsh\gmsh.exe', '..\Data\frame.geo -3', ExitCode, 60000, True);

  Gmsh.OpenFile('..\Data\frame.msh');
  Gmsh.ReadMesh;
  Gmsh.Close;
  ...
  begin
    ...
    begin
      ...
      raise Exception.Create('Error: Only beams supported.');
  ...
  // (positive-definite) matrix type - PardisoSolve(...,True). See
  ...
  ShellExecute(Handle,'open', 'c:\gmsh\gmsh.exe', '..\Data\frame.pos', nil, SW_SHOWNORMAL) ;
